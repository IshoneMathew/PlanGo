...
class _ItineraryPreviewScreenState extends State<ItineraryPreviewScreen> {
  ...
  double get _transportCostPerDay => (_transportModes
      .firstWhere((m) => m['id'] == _transportMode)['price'] as int).toDouble();
  ...
  double get _guideTotal => _wantGuide && _selectedGuide != null
      ? _selectedGuide!.pricePerDay * widget.days : 0;

  double get _activitiesEstimate => 25.0 * widget.days;
  double get _foodEstimate => 20.0 * widget.days;

  double get _totalCost =>
      _hotelTotal + _transportTotal + _guideTotal + _activitiesEstimate + _foodEstimate;
      ...
      child: Row(children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: _overBudget ? AppColors.red.withOpacity(0.1) : AppColors.green.withOpacity(0.1),
            borderRadius: BorderRadius.circular(20)),
          child: Text(
            _overBudget ? '⚠️ Over budget' : '✓ In budget',
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text('AI Generated', style: GoogleFonts.spaceGrotesk(fontSize: 10, color: Colors.white30)),
        ]),
        const SizedBox(height: 12),
        Wrap(spacing: 8, children: [
          _SumPill(Icons.calendar_today_outlined, '${widget.days} days', const Color(0xFF00C6FF)),
          _SumPill(Icons.attach_money, '\$${widget.budget.toInt()} budget', const Color(0xFF71B280)),
          _SumPill(Icons.receipt_long_outlined, '\$${_totalCost.toInt()} est. total',
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Over budget by \$${_budgetDiff.toInt()}',
              style: GoogleFonts.spaceGrotesk(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.red)),
          const SizedBox(height: 2),
          Text('Try selecting cheaper hotels below, a different transport mode, or skip the tour guide to bring costs down.',
              style: GoogleFonts.spaceGrotesk(fontSize: 11, color: AppColors.red.withOpacity(0.8), height: 1.5)),
        ])),
      ]),
    );
  }
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: sel ? [BoxShadow(color: AppColors.primary.withOpacity(0.25), blurRadius: 6)] : null),
              child: Row(children: [
                Icon(m['icon'] as IconData, color: sel ? Colors.white : AppColors.textSecondary, size: 18),
                const SizedBox(width: 8),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                  Text(m['label'] as String, style: GoogleFonts.spaceGrotesk(fontSize: 12, fontWeight: FontWeight.w600,
                      color: sel ? Colors.white : AppColors.textPrimary)),
                  Text('\$${m['price']}/day', style: GoogleFonts.spaceGrotesk(fontSize: 10,
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ClipRRect(
            ...
            child: Stack(children: [
              Image.network(day.imageUrl, height: 100, width: double.infinity, fit: BoxFit.cover,
                  errorBuilder: (_, __, ___) => Container(height: 100, color: Colors.grey.shade200)),
              Container(height: 100, decoration: const BoxDecoration(gradient: AppColors.darkGradient)),
              Positioned(bottom: 10, left: 14, right: 14, child: Row(children: [
                ...
                const SizedBox(width: 8),
                Text(day.lastStop, style: GoogleFonts.spaceGrotesk(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.white)),
              ])),
            ]),
          ),
          // Activities
              ...
              ...day.activities.asMap().entries.map((e) => Padding(
                ...
                child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Expanded(child: Text(e.value,
                      style: GoogleFonts.spaceGrotesk(fontSize: 13, color: AppColors.textPrimary, fontWeight: FontWeight.w500))),
                ]),
              )),
  ...
  Widget _noHotelsChip() => Container(
    ...
    child: Text('No hotels found for this location — you can add one manually after booking.',
        style: GoogleFonts.spaceGrotesk(fontSize: 12, color: AppColors.textSecondary)),
  );
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(14),
                border: Border.all(
                  color: isSelected ? AppColors.primary : isOverBudget ? AppColors.warning.withOpacity(0.5) : AppColors.divider,
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ClipRRect(
                  ...
                  child: Stack(children: [
                    Image.network(hotel.imageUrl, height: 80, width: double.infinity, fit: BoxFit.cover,
                        errorBuilder: (_, __, ___) => Container(height: 80, color: Colors.grey.shade200,
                            child: const Icon(Icons.hotel, color: AppColors.grey))),
                    if (isOverBudget)
                      Positioned(top: 6, right: 6,
                          child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                              ...
                              child: Text('Over budget', style: GoogleFonts.spaceGrotesk(fontSize: 9, fontWeight: FontWeight.w700, color: Colors.white)))),
                    if (hotel.freeCancellation)
                      Positioned(top: 6, left: 6,
                          child: Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                              decoration: BoxDecoration(color: AppColors.green, borderRadius: BorderRadius.circular(6)),
                              child: Text('Free cancel', style: GoogleFonts.spaceGrotesk(fontSize: 9, fontWeight: FontWeight.w700, color: Colors.white)))),
                    if (isSelected)
                ...
                Padding(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(hotel.name, style: GoogleFonts.spaceGrotesk(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.textPrimary),
  ...
  Widget _buildTourGuideSection() {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Text('Tour Guide', style: GoogleFonts.spaceGrotesk(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
        const Spacer(),
        Switch(value: _wantGuide, onChanged: (v) => setState(() => _wantGuide = v), activeColor: AppColors.primary),
      ]),
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              return GestureDetector(
                onTap: () => setState(() => _selectedGuide = isSelected ? null : guide),
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 150),
                  width: 175,
                  ...
                  child: Padding(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        CircleAvatar(radius: 24, backgroundImage: NetworkImage(guide.photoUrl),
                            backgroundColor: Colors.grey.shade200),
                        const SizedBox(width: 8),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(guide.name, style: GoogleFonts.spaceGrotesk(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                          Row(children: [
                            const Icon(Icons.star, color: AppColors.star, size: 11),
                            const SizedBox(width: 2),
                            Text('${guide.rating}', style: GoogleFonts.spaceGrotesk(fontSize: 10, color: AppColors.textSecondary)),
                          ]),
                        ])),
                        if (isSelected) const Icon(Icons.check_circle, color: AppColors.primary, size: 18),
                      ]),
                      const SizedBox(height: 8),
                      Text(guide.bio, style: GoogleFonts.spaceGrotesk(fontSize: 10, color: AppColors.textSecondary, height: 1.4),
                          maxLines: 3, overflow: TextOverflow.ellipsis),
                      const Spacer(),
                      Wrap(spacing: 4, children: guide.specialties.take(2).map((s) => Container(
                        padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                        decoration: BoxDecoration(color: AppColors.lightGrey, borderRadius: BorderRadius.circular(6)),
                        child: Text(s, style: GoogleFonts.spaceGrotesk(fontSize: 9, color: AppColors.textSecondary)),
                      )).toList()),
                      const SizedBox(height: 6),
                      Text('\$${guide.pricePerDay}/day · ${guide.tours} tours',
                          style: GoogleFonts.spaceGrotesk(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.primary)),
                    ]),
                  ),
                ),
  ...
  Widget _buildCostBreakdown() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 8)]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Cost Breakdown', style: GoogleFonts.spaceGrotesk(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
        const SizedBox(height: 14),
        _CostRow('🏨 Hotels (${widget.days - 1} nights)', _hotelTotal),
        _CostRow('🚗 Transport (${_transportModes.firstWhere((m) => m['id'] == _transportMode)['label']})', _transportTotal),
        ...
        const Divider(height: 20, color: AppColors.divider),
        Row(children: [
          Text('Estimated Total', style: GoogleFonts.spaceGrotesk(fontSize: 15, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
          const Spacer(),
          Text('\$${_totalCost.toInt()}', style: GoogleFonts.spaceGrotesk(fontSize: 18, fontWeight: FontWeight.w900,
              color: _overBudget ? AppColors.red : AppColors.primary)),
        ]),
        const SizedBox(height: 8),
        Row(children: [
          Text('Your budget', style: GoogleFonts.spaceGrotesk(fontSize: 12, color: AppColors.textSecondary)),
          const Spacer(),
          Text('\$${widget.budget.toInt()}', style: GoogleFonts.spaceGrotesk(fontSize: 13, fontWeight: FontWeight.w600,
              color: AppColors.textSecondary)),
        ]),
        if (_overBudget) ...[
          const SizedBox(height: 8),
          Container(padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(color: AppColors.red.withOpacity(0.06), borderRadius: BorderRadius.circular(10)),
              child: Row(children: [
                const Icon(Icons.info_outline, color: AppColors.red, size: 14),
                const SizedBox(width: 6),
                Expanded(child: Text('You are \$${_budgetDiff.toInt()} over your budget. Select cheaper hotels or a different transport mode.',
                    style: GoogleFonts.spaceGrotesk(fontSize: 11, color: AppColors.red, height: 1.4))),
              ])),
        ] else ...[
          const SizedBox(height: 8),
          Container(padding: const EdgeInsets.all(10),
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 6),
                Text('Great! You have \$${(widget.budget - _totalCost).toInt()} left for extras.',
                    style: GoogleFonts.spaceGrotesk(fontSize: 11, color: AppColors.green)),
              ])),
        ],
      ]),
    );
  ...
  Widget _buildBookButton(BuildContext context) {
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        if (_overBudget) Padding(
          padding: const EdgeInsets.only(bottom: 8),
          child: Text('⚠️ This trip exceeds your budget by \$${_budgetDiff.toInt()} — you can still book it.',
        ...
        Row(children: [
          Expanded(
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: _overBudget ? AppColors.warning : AppColors.dark,
          ...
          GestureDetector(
            ...
            child: Container(width: 52, height: 52,
                ...
                child: const Icon(Icons.bookmark_border, color: AppColors.primary, size: 22)),
          ),
        ]),
      ]),
    );
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('🎉 Trip booked and saved to My Trips!'), backgroundColor: AppColors.green));
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Booking failed: $e'), backgroundColor: AppColors.red));
      }
    }
  }
}
...
class _TimelineItem extends StatelessWidget {
  ...
  const _TimelineItem({required this.name, required this.sub, required this.isFirst, required this.isLast, required this.showLine});
  @override
  Widget build(BuildContext context) {
    ...
    return IntrinsicHeight(child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(name, style: GoogleFonts.spaceGrotesk(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.white)),
          Text(sub, style: GoogleFonts.spaceGrotesk(fontSize: 10, color: Colors.white54),
              maxLines: 1, overflow: TextOverflow.ellipsis),
        ]),
      )),
    ]));
...
class _CostRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    padding: const EdgeInsets.only(bottom: 8),
    child: Row(children: [
      Expanded(child: Text(label, style: GoogleFonts.spaceGrotesk(fontSize: 13, color: AppColors.textSecondary))),
      Text('\$${amount.toInt()}', style: GoogleFonts.spaceGrotesk(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
    ]),
  );
}
